...
if __name__ == '__main__':
    ...
    for step_size in range(start_step, finish_step + 1):
        ...
        for i in tqdm(range(len(pcds) - step_size - 1)):
            trans_prel = pairwise_registration_ransac(pcds[i + step_size], pcds[i])
            trans_result = refine_registration(pcds[i + step_size], pcds[i], trans_prel)
            transformations.append(trans_result)

        np.save(sys.argv[1].split("/")[1] + "{0}step_trans.npy".format(step_size), np.asarray(transformations))
